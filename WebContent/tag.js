...
(function($) {
	...
	$(document).ready(function() {
		...
		try {
			...
			//var recognition = spoke.recognizer.Recognizer($('#speech-mic'));
		...
		if (!(nuturl.indexOf("Eval")==-1)){
			var thead = $('<thead><tr class="active"><th>Food</th><th>Quantity</th><th>USDA Hits</th><th>Correct Quantity?</th><th>Correct USDA hit?</th><th>Correct Image?</th></tr></thead>');	
		} else {
...
//			responseNum+=1;
//			console.log("increment response num to: " + responseNum);
//			
//			//checks to see if the extra boxes at the end have been added
//			extra_boxes_added = false;
//
//			// enable submit if all checks pass and responseNum > 4
//			// only one required for evaluating the system
//			// ensure at least one food
//			console.log("foodNum: " + foodNum + ", responseNum: " + responseNum);
//			console.log(((responseNum > 4) || (!(nuturl.indexOf("Eval")==-1) & responseNum > 1)));
//
//			if (((responseNum > 4) || (!(nuturl.indexOf("Eval")==-1) & responseNum > 1)) & foodNum > 0){
//			//if ((responseNum > 1) || (!(nuturl.indexOf("Eval")==-1) & responseNum > 1)){
//				console.log("ready to submit!");

				// when ready to submit, add table for substitutions and deletions
//				if (!(nuturl.indexOf("Eval")==-1) && !extra_boxes_added){
//					var table2 = $('<table class=center></table>')
//					.attr('border', '1')
//					.css({textAlign: 'center'})
//					.addClass('table table-bordered');
//					$('#deletions').append(table2); 
//					var thead2 = $('<thead><tr class="active"><th>How many foods were missed?</th><th>How many any-food words were labeled as foods?</th></tr></thead>');
...
//					table2.append(row);
					...
					//extra_boxes_added = true;
					//console.log("extra_boxes_added: " + extra_boxes_added);
...
//				if (!(nuturl.indexOf("Eval")==-1)){
//					$("#incompleteWarning").html("You'll need to record 2 valid meals with foods before you can submit. Keep going!");
//				}
...
//			// check that diary is not a repeat
			$.ajax({
				...
				async: false}).done(function(data) {
					...
					if(comp==1 || comp==0){
						...
						// files located in /afs/csail.mit.edu/proj/slu/www/data/corpus/data.txt
						$.ajax({
		...
		function displayTable(data){
			...
			var backUpImages = data.backUpImages;
			...
			for(var food in dependencies){
				...
				if (foodItemsToRows[foodNoNums] !=null && foodItemsThisEntry[foodNoNums]==null) {
					...
					if (hits.length == 1) {
						...
						//if the food item does not hava USDA food ID
						//fallback on Rachael's image search method
						if (hits[0].foodID=="-1") {
							console.log("food not in USDA database");
							...
							backUpImageEncoding = backUpImages[foodNoNums];
							System.out.println("backUpImageEncoding: " + foodNoNums + ": " + backUpImageEncoding)
						}else {
					...
					//console.log(images);
					console.log("images size: " + Object.keys(images).length);
					...
					// add image results to row
					$("#dependencies tr:nth-child("+tableRow+") td:nth-child(1)").append(img);			
					
					// adding delete button
					$("#dependencies tr:nth-child("+tableRow+") td:nth-child(3)").append("<input type='button' title='Delete Food' id='delete-btn' value='X'/>");
					
					// deleting the row
					$('input[type="button"]').click(function(e){
						console.log("tablerow before in delete: " + tableRow);
						$(this).closest('tr').remove()
						if(tableRow > 0){
							tableRow--;
						}
						
						console.log("tablerow after in delete: " + tableRow);
						})

					// add db results to row
					...
					// create substition and deletion buttons
					foodNum += 1;
					...
					if (!(nuturl.indexOf("Eval")==-1)){
						...
						yesNoRadioBtns("radio_quantity", 4, foodNum);

						// create USDA hit radio buttons
						yesNoRadioBtns("radio_USDA", 5, foodNum);

						// create image hit radio buttons
						yesNoRadioBtns("radio_Image", 6, foodNum);
						...
						var thead2 = $('<thead><tr class="active"><th>How many foods were missed?</th><th>How many non-food words were labeled as foods?</th></tr></thead>');
						table2.append(thead2);
						//table2.append("<td class=columnHeader><strong>Were any foods missed?</strong></td>");
						//table2.append("<td class=columnHeader><strong>Were any non-food words labeled as foods?</strong></td>");
						var row = $('<tr id="checkboxes-delete-subst">" "</tr>'); 
					...
					if (hits.length==1) {
						console.log("hits length = 1 table row: " + tableRow);
		...
		function yesNoRadioBtns(name, col, foodNum){
			//console.log("creating radio buttom for " + name + ", col: " + col + " , foodNum: " + foodNum);
...
//					makeSubsequentUSDAListeners (item, tableRow, hits, featureChosen, updatedResult.weights,brand);
		...
		function changePhoto(foodID, tableRow){
			...
			$.getJSON(nuturl+'UpdatePhoto2'+'?jsonp=?', {'foodID' : foodID},
					function(updatedResult){
				console.log("went to Update Photo2 to get new photo")
...
//		tag("I had cereal and milk");
		...
		/*
		recognition.on('start.spoke.recognizer', function (event) {
            console.log('Spoke Recognizer start', event);
        });

		recognition.on('stop.spoke.recognizer', function (event) {
            console.log('Spoke Recognizer stop', event);
        });
        
		recognition.on('finalResult.spoke.recognizer', function (event, result) {*/
