...
public class GetAttributesCRF {
	...
	static Map<String, ArrayList<Segment>> getAttributeDeps(Sentence sentence, NLPData segmentation, ArrayList<CRFToken> foodItems, String labelRep) throws IOException {		
		...
		if (useFST){
			// get list of segments and labels
			Object[] segsAndLabels = GetAttributesFST.getSegments(sentence, segmentation);
			segs = (ArrayList<Segment>) segsAndLabels[1];
		
			// run CRF++				
			CRFout = CRF.getOutputFST(segmentation.tokens, segmentation.labels, null, labelRep);
		} else {
		...
		if (useFST){
			segmentDeps = GetAttributesFST.getAttributesFromFST(CRFoutList, segmentDeps, segs, segmentation);
		} else {
